...
/** Live countdown — ticks every second until targetIso */
...
/** Podium + ranked list shared between both tabs */
...
export default function Leaderboard() {
  ...
  // All-time data — fetched lazily on first tab switch
  ...
  // ── Load weekly data + quiz title on mount ─────────────────────────────────
  useEffect(() => {
    ...
    async function load() {
      try {
        const [lbRes, quizRes] = await Promise.all([
          api.get(`/quizzes/${quizId}/leaderboard?scope=weekly`),
          api.get(`/quizzes/${quizId}`),
        ]);
        if (cancelled) return;
        setWeeklyEntries(lbRes.data.entries ?? []);
  ...
  // ── Load all-time data lazily when that tab is first opened ───────────────
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        const res = await api.get(
          `/quizzes/${quizId}/leaderboard?scope=alltime`,
        );
        if (cancelled) return;
  ...
  }, [tab, alltimeLoaded, authLoading, quizId]);
  ...
  return (
    <div className="min-h-screen bg-tint">
      <div className="max-w-xl mx-auto px-4 pt-6 pb-10">
        ...
        {/* Weekly info bar — only on weekly tab */}
        {tab === "weekly" && (
        ...
        {/* All-time info bar — only on all-time tab */}
        {tab === "alltime" && (
